...
#define SCROLL_SECONDS 1.0  // FIXED: Scroll 1 second per keypress
...
// FIXED: Improved PRF detection
static double calculate_prf(float *data, int frames, double time_per_frame) {
    if (frames <= 0 || data == NULL || time_per_frame <= 0) return 0.0;
    
    // Average across frequency bins to get time series power
    ...
    // Calculate average power per time frame
    ...
    // Find peaks above threshold (more sensitive detection)
    double threshold = mean + 1.0 * std;  // Lower threshold for better detection
    int *peaks = malloc(actual_frames * sizeof(int));
    if (!peaks) { free(time_series); return 0.0; }
    
    int peak_count = 0;
    int above = 0;
    int min_gap = 2;  // Minimum frames between peaks
    ...
    // Calculate PRF from peak intervals
    double prf = 0.0;
    if (peak_count >= 2) {
        ...
        for (int i = 1; i < peak_count; i++) {
            ...
            if (interval > 0.001 && interval < 10.0) {  // Reasonable PRF range
...
static void match_known_signals(SDRAnalyzer *a, double center_freq, double bandwidth) {
    ...
    struct {
        ...
        {"5G NR Downlink", 2300e6, 2690e6, 5e6, 100e6},
        {"5G NR Uplink", 2300e6, 2690e6, 5e6, 100e6},
        {"4G LTE Downlink", 2300e6, 2690e6, 1.4e6, 20e6},
        {"4G LTE Uplink", 2300e6, 2690e6, 1.4e6, 20e6},
        {"WiFi 802.11n/g", 2400e6, 2483.5e6, 20e6, 40e6},
        {"WiFi 802.11ac/ax", 2400e6, 2483.5e6, 20e6, 160e6},
        {"Logitech Wireless", 2400e6, 2483.5e6, 500e3, 2e6},
        {"Zigbee", 2400e6, 2483.5e6, 2e6, 2e6},
        {"Bluetooth Classic", 2400e6, 2483.5e6, 1e6, 3e6},
        {"Bluetooth LE", 2400e6, 2483.5e6, 1e6, 2e6},
    };
...
static void manage_cache_windows(SDRAnalyzer *a, int target_frame) {
    ...
    for (int i = a->num_cache_windows - 1; i >= 0; i--) {
        ...
        if (dist > max_distance && a->num_cache_windows > PRECACHE_WINDOWS) {
            if (a->cache_windows[i].data) {
                free(a->cache_windows[i].data);
                a->cache_windows[i].data = NULL;
            }
            a->cache_windows[i].valid = 0;
            pthread_mutex_unlock(&a->cache_windows[i].mutex);
...
static int find_or_create_cache_window(SDRAnalyzer *a, int start_frame) {
    ...
    if (a->num_cache_windows >= MAX_CACHE_WINDOWS) {
        ...
        if (a->cache_windows[lru_idx].data) {
            free(a->cache_windows[lru_idx].data);
            a->cache_windows[lru_idx].data = NULL;
        }
    ...
    while (!a->quit_thread) {
        ...
        for (int i = start_frame; i < end_frame; i++) {
            ...
            if (frames_loaded % LOAD_BATCH_SIZE == 0) {
                pthread_mutex_lock(&a->cache_mutex);
                a->load_progress = (frames_loaded * 100) / num_frames;
                a->needs_redraw = 1;
                pthread_mutex_unlock(&a->cache_mutex);
            }
        }
...
static void request_cache_load(SDRAnalyzer *a, int start_frame) {
    ...
    if (a->last_cached_frame >= 0) {
        int diff = abs(start_frame - a->last_cached_frame);
        if (diff < 100) {
            pthread_mutex_unlock(&a->cache_mutex);
            return;
        }
...
static void ensure_cache(SDRAnalyzer *a, int frame_idx) {
    ...
    pthread_mutex_lock(&a->cache_mutex);
    if (a->last_cached_frame >= 0) {
        int diff = abs(frame_idx - a->last_cached_frame);
        if (diff < a->cache_frames / 2) {
            pthread_mutex_unlock(&a->cache_mutex);
            return;
        }
    }
...
static void update_zoom_display(SDRAnalyzer *a) {
    if (a->zoom_label) {
        char zoom_text[64];
        snprintf(zoom_text, sizeof(zoom_text), "Zoom: %.1fs", a->visible_seconds);
        gtk_label_set_text(GTK_LABEL(a->zoom_label), zoom_text);
    }
}
...
static gboolean ui_update_timer(gpointer user_data) {
    ...
    int is_scrolling = a->is_scrolling;
    pthread_mutex_unlock(&a->cache_mutex);
    ...
    if (needs_redraw && !is_scrolling) {
        ...
        if (loading) {
            ...
            char info[512];
            snprintf(info, sizeof(info), "Pos: %.2f/%.1f s | Zoom: %.1fs | FFT: %d",
                a->scroll_position, a->total_duration, a->visible_seconds, a->fft_size);
...
static void on_zoom_in(GtkButton *button, gpointer user_data) {
    ...
    if (new_visible != a->visible_seconds) {
        ...
        // FIXED: Recalculate cache frames based on new zoom level
        ...
        // FIXED: Force cache reload for new zoom level
        int current_frame = (int)(a->scroll_position * a->frames_per_second);
        pthread_mutex_lock(&a->cache_mutex);
        a->last_cached_frame = -1;  // Force reload
        pthread_mutex_unlock(&a->cache_mutex);
...
// FIXED: Simple scroll function - only scrolls, nothing else
static void on_scroll(SDRAnalyzer *a, double delta) {
    ...
    if (new_pos != a->scroll_position) {
        ...
        // FIXED: Just mark for redraw, cache loading happens separately
        if (a->waterfall_drawing) gtk_widget_queue_draw(a->waterfall_drawing);
        
        // Schedule cache load for after scroll completes
...
static gboolean on_key_press(GtkWidget *widget, GdkEventKey *event, gpointer user_data) {
    ...
    switch (event->keyval) {
        case GDK_KEY_Up:
            // FIXED: Only scroll up 1 second, nothing else
            on_scroll(a, -SCROLL_SECONDS);
            break;
        case GDK_KEY_Down:
            // FIXED: Only scroll down 1 second, nothing else
            on_scroll(a, SCROLL_SECONDS);
            break;
        case GDK_KEY_plus:
        case GDK_KEY_equal:
...
static gboolean on_waterfall_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
    ...
    if (event->button == 1) {
        ...
        a->selection.freq_min = freq;
        a->selection.freq_max = freq;
        a->selection.time_min = time;
        a->selection.time_max = time;
...
static gboolean on_waterfall_button_release(GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
    ...
    if (event->button == 1 && a->selection.active) {
        ...
        if (freq < a->selection.freq_min) a->selection.freq_min = freq;
        else a->selection.freq_max = freq;
        
        if (time < a->selection.time_min) a->selection.time_min = time;
        else a->selection.time_max = time;
        
        printf("Selection: %.3f-%.3f MHz, %.2f-%.2f s\n",
               a->selection.freq_min/1e6, a->selection.freq_max/1e6,
               a->selection.time_min, a->selection.time_max);
...
static void on_analyze_button(GtkButton *button, gpointer user_data) {
    ...
    if (a->selection.active) {
        ...
        printf("No selection made. Click and drag on waterfall to select a region.\n");
    }
}
...
static void on_screenshot_button(GtkButton *button, gpointer user_data) {
    ...
    if (a->waterfall_surface) {
        cairo_surface_write_to_png(a->waterfall_surface, filename);
...
static void analyze_selection(SDRAnalyzer *a) {
    ...
    if (segment_len < 8192) {
        printf("Error: Selection too short\n");
        return;
    }
    ...
    int fft_size = 2048;
    int hop_size = (int)(fft_size * 0.75);
    ...
    // FIXED: Calculate PRF with better time resolution
    ...
    if (a->analysis.bandwidth < 5000) {
        strcpy(a->analysis.signal_type, "Narrowband");
    } else if (a->analysis.bandwidth < 1000000) {
        strcpy(a->analysis.signal_type, "Wideband");
    } else {
        strcpy(a->analysis.signal_type, "Very Wideband (OFDM)");
    }
    
    if (a->analysis.bandwidth < 3000) {
        strcpy(a->analysis.modulation, "AM/SSB/CW");
    } else if (a->analysis.bandwidth < 15000) {
        strcpy(a->analysis.modulation, "FM/NFM");
    } else if (a->analysis.bandwidth < 500000) {
        strcpy(a->analysis.modulation, "QAM/OFDM");
    } else {
        strcpy(a->analysis.modulation, "Wideband OFDM");
    }
    ...
    printf("PRF: %.2f Hz\n", a->analysis.prf);
    printf("Signal Type: %s\n", a->analysis.signal_type);
    printf("Modulation: %s\n", a->analysis.modulation);
    ...
    if (a->popup_window) {
        gtk_widget_destroy(a->popup_window);
    }
    ...
    snprintf(stats_text, sizeof(stats_text),
        ...
        a->analysis.freq_min/1e6, a->analysis.freq_max/1e6,
        a->analysis.bandwidth/1e3,
        a->analysis.center_freq/1e6,
        a->analysis.time_min, a->analysis.time_max,
        a->analysis.duration,
        a->analysis.entropy,
        a->analysis.prf,
        a->analysis.avg_power,
        a->analysis.max_power,
        a->analysis.signal_type,
        a->analysis.modulation);
    ...
    for (int i = 0; i < a->analysis.match_count && i < 5; i++) {
        char match_text[128];
        snprintf(match_text, sizeof(match_text), "%d. %s (%d%%)", 
                 i+1, a->analysis.matches[i], a->analysis.match_confidence[i]);
...
static gboolean draw_waterfall(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
    ...
    // FIXED: Calculate frames based on current zoom level (visible_seconds)
    ...
    int frames_needed = (int)(a->visible_seconds * a->frames_per_second);
    if (end_frame - start_frame < frames_needed) {
        start_frame = a->total_frames - frames_needed;
    ...
    // FIXED: Scale frames to fill the display height based on zoom level
    double frame_scale = (double)height / actual_frames;
    if (frame_scale < 1.0) frame_scale = 1.0;
    
    for (int i = 0; i < actual_frames; i++) {
        int frame_idx = vis_start + i;
        int cache_idx = frame_idx - cache_start;
        
        int y_start = (int)((actual_frames - i - 1) * frame_scale);
        int y_end = (int)((actual_frames - i) * frame_scale);
...
int main(int argc, char *argv[]) {
    ...
    analyzer.scroll_seconds = SCROLL_SECONDS;  // FIXED: 1 second scroll
    ...
    printf("\n========================================\n");
    printf("SDR Waterfall with Zoom Controls\n");
    printf("========================================\n");
    printf("FFT Size: %d bins\nDefault Zoom: %.1f seconds\n", analyzer.fft_size, analyzer.visible_seconds);
    printf("Controls:\n");
    printf("  Up/Down    : Scroll 1 second\n");
    printf("  +/-        : Zoom in/out\n");
    printf("  Mouse drag : Select region\n");
    printf("  A          : Analyze selection\n");
    printf("========================================\n\n");
    ...
    for (int i = 0; i < analyzer.num_cache_windows; i++) {
        ...
        pthread_mutex_destroy(&analyzer.cache_windows[i].mutex);
    }
